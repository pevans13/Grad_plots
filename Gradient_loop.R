...
for (i in 5:(ncol(Gradient))){
    if(is.integer(Gradient[[i]])==T){
      ...
      model.list<-list(Modnull1,Mod_cont,Mod_cont_NL)
      Top_model<-model.list[[which.min(sapply(X = model.list, FUN = AICc))]]
...
Plot1<-ggplot(data=Fungi_s,aes(x=Plot,y=Mean))+geom_pointrange(aes(ymin=Mean+SE,ymax=Mean-SE), width = 1)
Plot1
